...
In this section we compute the energy bands, to compare to the later QSGW result.
For this tutorial use the following symmetry lines.  Copy the contents of the
box below into _syml.fe_{: style="color: green"}.
...
To get the Fermi level corresponding to the given density without overwriting _rst.fe_{: style="color: green"}, do:

~~~
$ lmf fe --quit=band
~~~
Note that the Fermi level is **-0.000599**.  This is saved in file _wkp.fe_{: style="color: green"}.
Make the energy bands with
...
The second command renames the file for future use.
