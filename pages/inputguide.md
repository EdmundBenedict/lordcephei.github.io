...
AUTOBAS\_MTO | i | lmf, lmfgwd | Y | 0 | Read parameters RSMH,EH,RSMH2,EH2 that control which LMTO basis functions enter the basis.<br><br>Once initial values have been generated you can tune these parameters automatically for the solid, using lmf with the --optbas switch; see Building_FP_input_file.html and FPoptbas.html.<br><br>The --optbas step is not essential, especially for large basis sets, but it is a way to improve on the basis without increasing the size.<br><br>There are the following choices for MTO:<br><br>0 Parameters not read from _basp.ext_{: style="color: green"}; they are specified in the input file ctrl.ext.<br>1 or 3: 1-κ parameters may be read from the basis file _basp.ext_{: style="color: green"}, if they exist<br>2 or 4: 2-κ parameters may be read from the basis file _basp.ext_{: style="color: green"}, if they exist<br>1 or 2: Parameters read from ctrl.ext take precedence over _basp.ext_{: style="color: green"}<br>3 or 4: Parameters read from _basp.ext_{: style="color: green"} take precedence over those read from ctrl.ext.
