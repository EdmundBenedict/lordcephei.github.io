...
PWMODE | i | lmf, lmfgwd | Y | | Controls how APWs are added to the LMTO basis.  &nbsp;1s digit:1. LMTO basis only<br>2. Mixed LMTO+PW<br>Examples: <br>fp/test/test.fp srtio3 and fp/test/test.fp felz 4<br>3. PW basis only<br>&nbsp;10s digit:0. PW basis fixed (less accurate, but simpler)<br>1. PW basis k-dependent. Symmetry-consistent but basis and basis dimension depend on k.<br>Example: fp/test/test.fp te</li></ol>{:/}
...
RDSIG | i | lmf, lmfgwd, lm, lmgf | Y | 0 | Controls how the QSGW self-energy &Sigma;<sup>0</sup> substitutes for the LDA exchange correlation functional.<br>Note: the GW codes store &Sigma;<sup>0</sup>&minus;<i>V<sub>xc</sub></i><sup>LDA</sup> in file *sigm.ext*{: style="color: green"}.<br>1s digit:<br>&nbsp;0 do not read &Sigma;<sup>0</sup><br>&nbsp;1 read file *sigm.ext*{: style="color: green"}, if it exists, and add it to the LDA potential<br>&emsp;&nbsp;Add 2 to symmetrize **sigm**<br>&emsp;&nbsp;Add 4 to include Re(**sigm**(T)) only<br>10s digit:<br>0&nbsp; simple interpolation<br>1&nbsp; approximate high energy parts of **sigm** by diagonal (strongly recommended)<br>3&nbsp; interpolate **sigm** by LDA eigenvectors (no longer supported)<br>In this mode use 100s digit for # interpolation points.<br>Add:<br>&nbsp;10000 to indicate the sigma file was stored in the full BZ (no symmetry operations are assumed)<br>&nbsp;20000 to use the minimum neighbor table (only one translation vector at the surfaces or edges; cannot be used with symmetrization)<br>&nbsp;40000 to allow file mismatch between expected k-points and file values.<br><br>Note: Some options can also be supplied through the command-line argument **--rsig**.
RSSTOL | r | ALL | Y | 5e-6 | Max tolerance in Bloch sum error for real-space &Sigma;.<br><br>Unless otherwise specified (by e.g. --rsig~rs), &Sigma; is read in k-space and is immediately converted to real space by inverse Bloch transform.<br>The r.s.form is forward Bloch summed and checked against the original k-space &Sigma;.<br>If the difference exceeds RSSTOL the program will abort.<br>The conversion should be exact to machine precision unless the range of &Sigma; is truncated.<br>You can control the range of real-space &Sigma; with RSRNGE below.
RSRNGE | r | ALL | Y | 5 | Maximum range of connecting vectors for real-space &Sigma; (units of ALAT)
...
QASA | i | ASA | Y | 3 | A parameter specifying the definition of ASA moments [<i>Q</i><sub>0</sub>,<i>Q</i><sub>1</sub>,<i>Q</i><sub>2</sub>](/docs/code/asaoverview/#generation-of-the-sphere-potential-and-energy-moments-q); see lmto documentation<br>0. Methfessel conventions for 2nd gen ASA moments <i>Q</i><sub>0</sub>,<i>Q</i><sub>1</sub>,<i>Q</i><sub>2</sub><br>1. <i>Q</i><sub>2</sub> = coefficient to $$\dot{\phi}^2{−}p\phi^2$$ in sphere.<br>2. Q1,Q2 accumulated as coefficients to $$\left<\phi\dot{\phi}\right>$$ and $$\left<dot{\phi}^2\right>$$, respectively.<br>3. 1+2 (Stuttgart conventions)
...
SIGP[...] | r | lmf, lmfgwd | Y | | Parameters used to interpolate the self-energy &Sigma;. Used in conjunction with the GW package. See gw.html for description. Default: not used
SIGP\_MODE | r | lmf, lmfgwd | Y | 4 | Specifies the linear function used for matrix elements of &Sigma; at highly-lying energies. With recent implementations of the GW package 4 is recommended; it requires no input from you.
...
DELRE | r | lmfgwd | Y | .01, .04 | frequency mesh parameters definining the real axis mesh in the calculation of Im $$χ_0$$.  Sets dw and omg\_c in GWinput.<br>The <i>i</i><sup>th</sup> mesh point is given by:<br>$$ ω_i = dw × (i−1) + [dw × (i−1)]^2/omg_c/2 $$ <br>Points are approximately uniformly spaced, separated by dw, up to frequency omg\_c, around which point the spacing begins to increase linearly with frequency.<br>When generating a GWinput template, lmfgwd passes DELRE(1) to the dw tag and DELRE(2) to the omg\_c tag.<br>Note: the similarity to OPTICS\_DW used by the optics part of lmf and lm.
DELTA | r | lmfgwd | Y | -1e-4 | δ-function broadening for calculating $$ χ_0 $$, in atomic units.<br>Tetrahedron integration is used if DELTA<0.<br>When generating a GWinput template, lmfgwd passes DELTA to the delta tag.
