...
In this example we convert _fplot_{: style="color: blue"}-generated _fplot.ps_{: style="color: green"} into
...
so that when the figures is embedded it has the proper dimensions, without padding with whitespace.

This example adapts _fplot.ps_{: style="color: green"} generated from _plot.frames_{: style="color: green"} in
...
so that the figure resides at the top.  This is accomplished with **-shftm** instruction.
...
Run **fplot**{: style="color: blue"} again, now adding a shift:

    $ fplot -shftm=0-120,792-303 -f plot.frames

If you run the `gs` command above again, you should now see:

    %%BoundingBox: 0 605 433 792

The left edge is at 0, the top at 792 points.

Use the Inkscape program to convert from _.ps_{: style="color: green"} to _.svg_{: style="color: green"}.
(This example was constructed using a linux build of Inkscape 0.48.4 r9939.)
...
_fplot.svg_{: style="color: green"} can now be embedded without any filling around it.
